...
import { LESSONS, isLessonUnlocked } from "../lib/lessons.ts"
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {lessons.map((lesson) => {
                    ...
                    return (
                      <Card
                        key={lesson.id}
                        className={`transition-opacity ${!unlocked ? "opacity-50" : ""}`}
                      >
                        <CardHeader className="pb-2">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            {completed && <Badge variant="success">{t(lang, "lessons.done")}</Badge>}
                            ...
                            {!unlocked && <Badge variant="secondary">{t(lang, "lessons.locked")}</Badge>}
                          </div>
